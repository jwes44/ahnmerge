...
class AhnMerge(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.mysb=tk.Spinbox(self.topFrame, textvariable=self.sv, from_=1, to=999,width=4).grid(row=0, column=len(labs)-2)
...
#                        print (x,tm)
                    ...
                    if not mat:
                        plda[x-1] = [a1[x],'']
            if num not in self.ahn:
                self.ahn[num] = [[],[],[],[],[]]
            nti(self.ahn[num][0],name)
#            print(plda)
            try:
                nti(self.ahn[num][1],str(plda[0][0]))
            except IndexError:
                pass
            try:
                nti(self.ahn[num][2],str(plda[0][1]))
            except IndexError:
                pass
            try:
                nti(self.ahn[num][3],str(plda[1][0]))
            except IndexError:
                pass
            try:
                nti(self.ahn[num][4],str(plda[1][1]))
            except IndexError:
                pass
        self.showahn()
