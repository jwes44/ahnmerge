...
#        c.execute('SELECT name,latitude,longitude,countrycode,cc2,admin1code,admin2code,admin3code,admin4code,geo.geonameid,featureclass,featurecode FROM alternateNamesV2,geo WHERE (alternatename=? AND alternateNamesV2.geonameid=geo.geonameid) OR name=? ', [addr[0], addr[0]])
    tadd = fixadd(addr[0])
    c.execute('SELECT name,latitude,longitude,countrycode,cc2,admin1code,admin2code,admin3code,admin4code,geonameid,featureclass,featurecode FROM geonames WHERE name LIKE ?', [tadd])
    ret = c.fetchall()
    c.execute('SELECT name,latitude,longitude,countrycode,cc2,admin1code,admin2code,admin3code,admin4code,geonames.geonameid,featureclass,featurecode FROM alternateNamesV2,geonames WHERE (alternatename LIKE ? AND alternateNamesV2.geonameid=geonames.geonameid AND isolanguage <> "iata")', [tadd])
    ret += c.fetchall()
#        print(len(ret), ret)
    geos=[]
...
#        print(len(ret), ret)
    ...
    for a in range(1,len(addr)):
        c.execute('SELECT name,featurecode,geonameid FROM geonames WHERE name LIKE ? AND featureclass="A"', [addr[a].replace('...','%')])
        ret1 = c.fetchall()
        c.execute('SELECT name,featurecode,geonames.geonameid FROM alternateNamesV2,geonames WHERE alternatename LIKE ? AND alternateNamesV2.geonameid=geonames.geonameid AND featureclass="A"', [addr[a]])
        ret1 += c.fetchall()
        geos=[]
