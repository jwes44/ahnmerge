...
#        c.execute('SELECT name,latitude,longitude,countrycode,cc2,admin1code,admin2code,admin3code,admin4code,geonames.geonameid,featureclass,featurecode,population FROM alternateNamesV2,geonames WHERE (alternatename=? AND alternateNamesV2.geonameid=geonames.geonameid) OR name=? ', [addr[0], addr[0]])
    ...
    ret += c.fetchall()
    c.execute('SELECT name,latitude,longitude,countrycode,cc2,admin1code,admin2code,admin3code,admin4code,geonames.geonameid,featureclass,featurecode,population FROM alternateNamesV2,geonames WHERE (alternatename LIKE ? AND alternateNamesV2.geonameid=geonames.geonameid AND isolanguage <> "iata")', [tadd])
    ret += c.fetchall()
#        print(len(ret), ret)
    geos=[]
