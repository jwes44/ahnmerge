...
#        print(len(ret), ret)
    for item in range(len(ret)):
        ...
        if len(country) == 1:
            ...
            o[ret[item][9]].featureclass = ret[item][10]
            o[ret[item][9]].featurecode = ret[item][11]
            if ret[item][10] == 'A':
                if ret[item][11] == 'ADM1':
                    o[ret[item][9]].num = 2
    ...
    for a in range(1,len(addr)):
        c.execute('SELECT name,featurecode,geonameid FROM geo WHERE name LIKE ? AND featureclass="A"', [addr[a].replace('...','%')])
        ret1 = c.fetchall()
        c.execute('SELECT name,featurecode,geo.geonameid FROM alternateNamesV2,geo WHERE alternatename LIKE ? AND alternateNamesV2.geonameid=geo.geonameid AND featureclass="A"', [addr[a]])
        ret1 += c.fetchall()
        geos=[]
